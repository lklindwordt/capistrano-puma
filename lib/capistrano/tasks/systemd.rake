  ...
  namespace :systemd do
    ...
    task :config do
      on roles(fetch(:puma_role)) do |role|
        ...
        sudo "mv #{fetch(:tmp_dir)}/#{unit_filename} #{fetch(:puma_systemd_conf_dir)}"	        systemd_path = fetch(:puma_systemd_conf_dir, git_plugin.fetch_systemd_unit_path)
        sudo "#{fetch(:puma_systemctl_bin)} daemon-reload"	        if fetch(:puma_systemctl_user) == :system
